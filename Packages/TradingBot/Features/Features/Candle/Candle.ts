...
export class CandleCircularBuffer extends CircularBuffer<ICandle> {
  ...
  getRangeOfCandles(
    ...
    while (result.length <= numToTake) {
      ...
      if (
        item &&
        item.pairPeriod.pair === pairPeriod.pair &&
        item.pairPeriod.period === pairPeriod.period
      )
        result.push(item);
...
export default class Candle {
  ...
  async startCandleProcesses(
    params: CandleProcess,
    model: Function
  ): Promise<void> {
    try {
  ...
  async processCandles(candles: any, model: Function): Promise<void> {
    for (const candle of candles || []) {
      try {
        ...
        this.generalStore.state.Time.add(
          "processCandles each candle Loop",
