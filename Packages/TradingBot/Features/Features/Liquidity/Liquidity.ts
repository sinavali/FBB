import {
    ...
    ISession,
    ICandle,
    AddNewLiquidityParams, LiquidityUsed,
...
import {LiquidityMode, Directions, Triggers} from "@shared/Types/Enums.ts";
...
export default class Liquidity {
     ...
     */
    private findHighestPoints(candles: ICandle[]): { highestPoint: number; touches: DateTime[]; } {
     ...
     */
    private findLowestPoints(candles: ICandle[]): { lowestPoint: number; touches: DateTime[]; } {
    ...
    private addLiquidity(params: AddNewLiquidityParams): ILiquidity {
        ...
        logger.info(JSON.stringify(newLiquidity));
        ...
        if (mode === LiquidityMode.BYSESSION || mode === LiquidityMode.BYWORKTIME) {
            if (newLiquids.length === 2) {
                if (isFirst[0]) {
                    ...
                    if (!pullbackResults.low) {
                        newLiquids.splice(1, 1);
                        logger.info(`failed by pullback: ${JSON.stringify(liquidityExists)}`);
                    }
                } else if (isFirst[1]) {
                    const liquidityExists = this.liquidities
                        .getAll()
                    ...
                    if (!pullbackResults.high) {
                        newLiquids.splice(0, 1);
                        logger.info(`failed by pullback: ${JSON.stringify(liquidityExists)}`);
                    }
                }
            }
    ...
    addNewUsed(triggerId: number, trigger: Triggers, liquidityId: number, usedModel: LiquidityUsed): void {
        const liquidity = this.liquidities.getById(liquidityId) as ILiquidity;
        const exists = liquidity.used.find(u => u.triggerId === triggerId && u.trigger === trigger);
        if (exists) return;

        liquidity.used.push(usedModel);
        this.liquidities.updateById(liquidityId, "used", liquidity.used);
        logger.info(`new used: ${JSON.stringify(trigger)}`);
        logger.info(`new used model: ${JSON.stringify(liquidity.used)}`);
    }

    updateUsed(triggerId: number, trigger: Triggers, liquidityId: number, usedModel: LiquidityUsed): void {
        const liquidity = this.liquidities.getById(liquidityId) as ILiquidity;
        const usedIndex = liquidity.used.findIndex(u => u.triggerId === triggerId && u.trigger === trigger);
        if (usedIndex === -1) return;

        liquidity.used[usedIndex] = usedModel;
        this.liquidities.updateById(liquidityId, "used", liquidity.used);
        logger.info(`update used: ${JSON.stringify(trigger)}`);
        logger.info(`new used model: ${JSON.stringify(liquidity.used)}`);
    }
    ...
    isExists(pairPeriod: PairPeriod, mode: LiquidityMode, time: number): ILiquidity | undefined {
