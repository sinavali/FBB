import {ICandle, ICOB, ILiquidity, ISignal, LiquidityUsed} from "@shared/Types/Interfaces/general.ts";
...
import {
    ...
    Directions,
    LiquidityUsedStatus, SignalStatus,
    Triggers,
    TriggerStatus,
...
export default class CandleOrderBlock {
    ...
    updateCOB(candle: ICandle) {
        ...
        founds.forEach((item) => {
            ...
            if (cob.direction === Directions.DOWN && candle.low <= cob.limit)
                this.makeCobTriggered(cob, candle);
            if (cob.direction === Directions.UP && candle.high >= cob.limit)
                this.makeCobTriggered(cob, candle);
        });
    ...
    private makeCobTriggered(cob: ICOB, candle: ICandle) {
        const index: number = this.orderBlocks
            .getAll().findIndex((e) => e.id === cob.id);
