...
export async function modelOne(generalStore: GeneralStore) {
  ...
  const data: ModelOneData = (await initiationPhaseModelOne(
    generalStore
  )) as ModelOneData;

  generalStore.state.Time.add(
    "initiationPhaseModelOne",
